...
def jugar_pvp(jugador1, jugador2):
    """
    Rutina para el modo Player vs Player.
    Contiene la lógica original del ciclo de juego de dos jugadores.
    """
    print(
...
def main():
    ...
    welcome_done = False
    jugador1, jugador2 = "Blancas", "Negras"  # Default names
    ...
    while True:
        ...
        if opcion == 1:  # Jugar
            ...
            if modo == 1:
                # Ask names on first PvP game if not customized yet
                if not welcome_done:
                    print("\n--- Configuramos la partida ---")
                    jugador1, jugador2 = pedir_nombres()
                    welcome_done = True

                jugar_pvp(jugador1, jugador2)
            elif modo == 2:
                play_vs_ai_test()
            elif modo == 3:
                jugar_ai_vs_ai()
            ...
            print("\n--- Cambiando nombres de jugadores (Modo PvP) ---")
            jugador1, jugador2 = pedir_nombres()
            welcome_done = True
            continue
