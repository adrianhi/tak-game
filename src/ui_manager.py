...
class TakTerminalUI:
    ...
    def _display_game_info(self):
        ...
        size = self.board.size
        print("─" * (size * 8 + 1))
        print(f"Turno actual: {'Blancas ⬛' if self.board.current_player == 'white' else 'Negras ⬜'}")
        print(f"\nPiezas disponibles:")
        print(f"  Blancas: {self.board.pieces['white']['flat']} piezas, {self.board.pieces['white']['capstones']} piedra angular")
        print(f"  Negras:  {self.board.pieces['black']['flat']} piezas, {self.board.pieces['black']['capstones']} piedra angular")
        print("=" * (size * 8 + 1))
        print("\nLeyenda:")
